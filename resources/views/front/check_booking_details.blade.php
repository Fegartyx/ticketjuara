...
<div class="relative flex flex-col w-full min-h-screen max-w-[640px] mx-auto bg-[#F8F8F9]">
    <div id="background" class="fixed w-full max-w-[640px] top-0 h-screen z-0">
        ...
        <img src="{{\Illuminate\Support\Facades\Storage::url($booking->ticket->thumbnail)}}" class="w-full h-full object-cover" alt="background">
    </div>
        ...
        <div class="flex items-center justify-between rounded-3xl p-[6px] pr-[14px] bg-white overflow-hidden">
            <div class="flex items-center gap-[14px]">
                <div class="flex w-[90px] h-[90px] shrink-0 rounded-3xl bg-[#D9D9D9] overflow-hidden">
                    <img src="{{\Illuminate\Support\Facades\Storage::url($booking->ticket->thumbnail)}}" class="w-full h-full object-cover" alt="thumbnail">
                </div>
                <div class="flex flex-col gap-[6px]">
                    ...
                    <p class="font-bold text-sm leading-[21px] text-[#F97316]">Rp {{ number_format($booking->ticket->price, 0, ',', '.')  }}</p>
                </div>
            </div>
            ...
            <div class="relative flex flex-col p-5 pb-[30px] gap-6">
                ...
                <div class="flex flex-col gap-[14px] shrink-0 h-full">
                    <div class="flex items-center justify-between">
                        ...
                        <p class="font-bold text-xl leading-[30px]">{{$booking->booking_trx_id}}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="font-bold text-sm leading-[21px]">Started At</p>
                        <p class="font-bold text-sm leading-[21px] text-[#FF1927]">{{$booking->started_at}}</p>
                    </div>
                    <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <p class="w-fit rounded-full p-[6px_12px] bg-[#07B704] font-bold text-xs leading-[18px] text-white">{{$booking->is_paid == 1 ? 'SUCCESS' : 'PENDING'}}</p>
                    </div>
                </div>
                ...
                <div class="flex items-center rounded-[20px] p-[10px] gap-[10px] bg-[#F8F8F9]">
                    ...
                    <p class="leading-[28px]">Gunakan kode <span class="font-bold">{{$booking->booking_trx_id}}</span> ketika menukarkan dengan tiket asli</p>
                </div>
            </div>
        </div>
    </main>
